#!/bin/bash
#SBATCH --job-name=train_rldx1_pt_image_robocasa_demospeedup_slow2_fast4_60k_globalbatch64_2gpu
#SBATCH --comment="RLDX-1 image DemoSpeedup slow2 fast4, action chunk 8, no gradient accumulation"
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab_premium
#SBATCH --wckey=project-short-name:sub_fast
#SBATCH --time=2-00:00:00
#SBATCH --output=out/%j-rldx1_img_robocasa_demospeedup.out
#SBATCH --error=out/%j-rldx1_img_robocasa_demospeedup.err
...
cd "$BASE_DIR"
DATA_DIR="${DATA_DIR:-$BASE_DIR/.work/datasets/robocasa_demospeedup_slow2_fast4}"
...
RUN_NAME="${RUN_NAME:-rldx1_img_robocasa_demospeedup_slow2_fast4_gb64_60k}"
...
echo "Data loader: demospeedup_sharded, workers per rank=${NUM_WORKERS:-12}, decoder threads=1"
...
exec "$BASE_DIR/.venv/bin/torchrun" --standalone --nnodes=1 --nproc_per_node="$NUM_GPUS" \
    ...
    --modality-config-path "$BASE_DIR/rldx/configs/data/robocasa_demospeedup_config.py" \
