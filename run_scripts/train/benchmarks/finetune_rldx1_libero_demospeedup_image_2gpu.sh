#!/bin/bash
#SBATCH --job-name=train_rldx1_pt_image_libero_demospeedup_60k_globalbatch32_2gpu
#SBATCH --comment="RLDX-1 image DemoSpeedup slow2 fast4, 8-step target, no gradient accumulation"
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=sjw_alinlab_premium
#SBATCH --wckey=project-short-name:sub_fast
#SBATCH --time=2-00:00:00
#SBATCH --output=out/%j-rldx1_img_libero_demospeedup.out
#SBATCH --error=out/%j-rldx1_img_libero_demospeedup.err
...
cd "$BASE_DIR"
DATA_DIR="${DATA_DIR:-$BASE_DIR/.work/datasets/libero_demospeedup_slow2_fast4}"
...
RUN_NAME="${RUN_NAME:-rldx1_img_libero_demospeedup_slow2_fast4_gb32_60k}"
OUTPUT_DIR="${OUTPUT_DIR:-${MODEL_OUTPUT_DIR:-$BASE_DIR/outputs}}"
[[ -f "$DATA_DIR/meta/speedup_action_stats.json" ]] || { echo "Incomplete DemoSpeedup dataset: $DATA_DIR" >&2; exit 1; }
...
echo "Data loader: demospeedup_sharded, workers per rank=${NUM_WORKERS:-8}, decoder threads=1"
...
exec "$BASE_DIR/.venv/bin/torchrun" --standalone --nnodes=1 --nproc_per_node="$NUM_GPUS" \
    ...
    --base-model-path "$BASE_MODEL_PATH" --video-length 1 --n-cog-tokens 64 --action-horizon 8 \
    --dataset-path "$DATA_DIR" --dataset-mode demospeedup_sharded \
    --dataloader-num-workers "${NUM_WORKERS:-8}" \
    --embodiment-tag GENERAL_EMBODIMENT \
    --modality-config-path "$BASE_DIR/rldx/configs/data/libero_demospeedup_config.py" \
