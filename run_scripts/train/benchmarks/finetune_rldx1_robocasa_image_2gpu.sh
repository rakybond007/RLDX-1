#!/bin/bash
#SBATCH --job-name=rldx1_img_robocasa_gb64
#SBATCH --nodes=1
#SBATCH --gpus=2
...
BASE_MODEL_PATH="${BASE_MODEL_PATH:-RLWRLD/RLDX-1-PT-IMG}"
NUM_GPUS=2
GLOBAL_BATCH_SIZE=64
GRAD_ACCUM="${GRAD_ACCUM:-1}"
MAX_STEPS="${MAX_STEPS:-60000}"
RUN_NAME="${RUN_NAME:-rldx1_img_robocasa_gb64}"
...
exec "$BASE_DIR/.venv/bin/torchrun" --standalone --nnodes=1 --nproc_per_node="$NUM_GPUS" \
    ...
    --base-model-path "$BASE_MODEL_PATH" --video-length 1 --n-cog-tokens 64 \
